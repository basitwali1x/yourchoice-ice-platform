...
export default function App() {
  ...
  return (
    <div className="h-screen overflow-hidden yci-frost-bg flex">
      ...
      <main className="flex-1 overflow-y-auto h-full relative custom-scrollbar bg-yci-bg0/90">

        {/* TOP COMMAND BAR (Fixed) */}
        <div className="sticky top-0 z-40 bg-yci-bg0/95 border-b border-white/5 backdrop-blur-md px-12 py-4 flex items-center justify-between shadow-2xl">
          <div className="flex items-center gap-8">
            <div className="font-black text-xl italic tracking-tighter text-white">[ ARCTIC ICE ]</div>
            <div className="h-8 w-px bg-white/10 mx-2"></div>

            {/* Metric 1: Money */}
            <div className="flex flex-col">
              <div className="text-[9px] uppercase font-bold text-yci-textMuted tracking-wider">Revenue Today</div>
              <div className="text-sm font-black text-green-400 flex items-center gap-1">
                ${stats.revenue_today.toLocaleString()} <span className="opacity-60 text-[10px]">vs ${(stats.revenue_today * 0.9).toFixed(0)}</span>
              </div>
            </div>

            <div className="h-8 w-px bg-white/10 mx-2"></div>

            {/* Metric 2: Broken? (Alerts) */}
            <div className="flex flex-col">
              <div className="text-[9px] uppercase font-bold text-yci-textMuted tracking-wider">Active Alerts</div>
              <div className={`text-sm font-black flex items-center gap-1 ${stats.pending_orders > 5 ? 'text-red-400' : 'text-white'}`}>
                {stats.pending_orders > 5 ? '🔴' : '🟢'} {stats.pending_orders > 5 ? '3 Critical' : 'All Clear'}
              </div>
            </div>

            <div className="h-8 w-px bg-white/10 mx-2"></div>

            {/* Metric 3: Drivers Moving? */}
            <div className="flex flex-col">
              <div className="text-[9px] uppercase font-bold text-yci-textMuted tracking-wider">Active Fleet</div>
              <div className="text-sm font-black text-white flex items-center gap-1">
                7<span className="opacity-50">/8 Units</span>
              </div>
            </div>

            <div className="h-8 w-px bg-white/10 mx-2"></div>

            {/* Metric 4: Unhappy Customers? */}
            <div className="flex flex-col">
              <div className="text-[9px] uppercase font-bold text-yci-textMuted tracking-wider">Missed Stops</div>
              <div className="text-sm font-black text-white flex items-center gap-1">
                0 <span className="text-[10px] text-yci-textMuted uppercase">Today</span>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-6">
            {/* Metric 5: Do I need to act? */}
            <div className={`px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest border ${stats.pending_orders > 10 ? 'bg-red-500/20 border-red-500 text-red-500 animate-pulse' : 'bg-green-500/10 border-green-500/30 text-green-400'}`}>
              {stats.pending_orders > 10 ? '⚠ ACTION REQUIRED' : 'SYSTEM HEALTHY'}
            </div>
            <YCIButton variant="secondary" className="!rounded-full !px-6 !py-2 !text-[10px] font-black" onClick={() => { setToken(null); localStorage.removeItem("admin_token"); }}>LOGOUT</YCIButton>
          </div>
        </div>
        ...
        <div className="p-12">
          <header className="flex justify-between items-end mb-16">
            <div>
              <h1 className="text-6xl font-black tracking-tighter uppercase">{nav === "Dashboard" ? "Control Room" : nav} <span className="text-yci-accent opacity-50 italic">{selectedDC ? regionalStats.find(r => r.id === selectedDC)?.name : 'GLOBAL'}</span></h1>
            ...
            nav === "Dashboard" && data ? (
              <div className="space-y-12 animate-fade-in pb-24">
                  ...
                  <div className="col-span-2">
                    <YCICard title="Live Operations Map" subtitle="Real-time fleet position and status">
                      <div className="h-[400px] bg-black/40 rounded-3xl border border-white/10 relative overflow-hidden group">
                        ...
                        {/* Pins */}
                        <div className="absolute top-[40%] left-[30%] flex flex-col items-center z-10">
                          <div className="h-4 w-4 rounded-full bg-green-500 shadow-[0_0_15px_rgba(74,222,128,1)] animate-pulse border-2 border-white"></div>
                          <div className="mt-1 bg-black/80 px-2 py-0.5 rounded text-[8px] font-black uppercase text-white shadow-xl">LC-03 (John)</div>
                        </div>
                        <div className="absolute top-[60%] left-[55%] flex flex-col items-center z-10">
                          <div className="h-4 w-4 rounded-full bg-yellow-500 shadow-[0_0_15px_rgba(234,179,8,1)] border-2 border-white"></div>
                          <div className="mt-1 bg-black/80 px-2 py-0.5 rounded text-[8px] font-black uppercase text-white shadow-xl">LF-01 (Mike) - LATE</div>
                        </div>
                      </div>
                    </YCICard>
                  </div>
                ...
                <div className="grid grid-cols-1">
                  <YCICard title="Revenue Velocity Heatmap" subtitle="Click any DC to drill down into localized performance">
                    <div className="mt-4">
